...
/*		//
	...
	public Map<String, Object> submitLogin(UUser entity, HttpServletRequest request) {
		...
		try {
			...
			data.put("createTime",login.getCreateTime().getTime());
			data.put("lastLoginTime",login.getLastLoginTime().getTime());
			data.put("sex",login.getSex());
			data.put("age",login.getAge());
			data.put("registerCount",login.getRegisterCount());
			data.put("userLogo",login.getUserLogo());
